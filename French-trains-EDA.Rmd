...
full_trains <- readr::read_csv("/home/sherwin/Downloads/full_trains.csv") %>%
...
november_2018 %>%
  ...
  labs( x = "Arrival Station",
        ...
        subtitle = "Stations with only one arriving/departing routes were lumped into 'Other' ")
...
full_trains %>%
  ...
  scale_y_continuous(labels = percent_format()) +
  expand_limits(y = 0) +
  labs(title = "Lyon Part Dieu", x = "Month", y = "Pct Late at Departure")
...
by_departure_station_month <- full_trains %>%
  group_by(departure_station = ifelse(service == "International" | 
                                      departure_station %in% c("Barcelona", "Madrid"),
...
by_departure_station_month %>%
  mutate(departure_station = fct_reorder(departure_station, ifelse(grepl("International", departure_station), 1, 0),mean)) %>%
  ...
  labs(x = "Month",
       ...
       subtitle = "Ordered by the average delay, international routes on the top ")
